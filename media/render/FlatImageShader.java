...
public class FlatImageShader extends IlluminationImageShader implements RenderingConstantsI {

   /**
    * Constructs a flat image shader associated with the receptor object argument.
    *
    * @param receptor the receptor object associated with this phong image shader.
    */

   protected FlatImageShader(VertexReceptor receptor) { super(receptor); }

   /**
    * Computes the color intensities of the pixel defined by the floating point arguments using the flat shading model and stores the result in the floating point array argument.
    *
   ...
   protected void computeLitPixel(float barycentricU, float barycentricV, float barycentricW, float[] sample) throws BidirectionalIlluminationException {
      ...
      float dotNV = normalX1 * view[0] + normalY1 * view[1] + normalZ1 * view[2];
	  ...
	  for(int i = 0; i < 16 && lightArray[i] != null; i++) {
         ...
         if(lightArray[i].getEnabled()) {
            ...
            lightArray[i].setReflectionCoordinates(position[0], position[1], position[2], normalX1, normalY1, normalZ1, reflection);
           ...
           float dotNL = incidence[0] * normalX1 + incidence[1] * normalY1 + incidence[2] * normalX1;
