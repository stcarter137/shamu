...
/** Class representing a basic image shader in the 3D rendering pipeline. Each instance contains four ouput registers that will store the output colors for an input pixel.
 *
 *  @author  Scott T. Carter
 *  @version 1.4
 */

public class BasicImageShader extends ImageShader implements RenderingConstantsI {

   /**
    * Constructs a basic image shader associated with the receptor object argument.
    *
    * @param receptor the receptor associated with this basic image shader.
    */

   protected BasicImageShader(VertexReceptor receptor) { super(receptor); }
   ...
   protected boolean set(RenderingAttributes renderingAttributes, float barycentricU, float barycentricV, float barycentricW, float screenX, float screenY, float deviceX, float deviceY, int pixelX, int pixelY) {
      ...
      if(screenX > -1.0f && screenX < 1.0f && screenY > -1.0f && screenY < 1.0f) {
         ...
         float[] sample = {emissiveCoefficientR, emissiveCoefficientG, emissiveCoefficientB};
